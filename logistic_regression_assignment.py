...
def metropolis_hastings_sample(X, y, m, S, nb_iter):
    ...
    sample_cur = max_lik_estimate(X, y).reshape(D,1)
    #sample_cur = 8*np.ones((D,1))

    i = 0
    while(i<nb_iter):
        sample_tmp = np.random.multivariate_normal(np.mean(sample_cur, axis=1), S).reshape(D, 1)
        p_tmp = np.exp(-neg_log_posterior(sample_tmp, X, y, m, S))
        p_cur = np.exp(-neg_log_posterior(sample_cur, X, y, m, S))
        ratio = p_tmp/p_cur
        u = np.random.uniform(0,1)
        if ratio>=u:
            samples[i] = sample_tmp.squeeze()
...
plt.xlabel('theta')
plt.savefig('Metropolis_samples.png')
plt.show()
##############